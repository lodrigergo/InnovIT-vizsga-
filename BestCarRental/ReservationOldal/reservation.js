document.addEventListener("DOMContentLoaded", function () {
    ...
    loginSubmitBtn.addEventListener("click", async function (event) {
        ...
        try {
            ...
            if (response.ok && data.status === "success") {
                ...
                if (savedImage) {
                    const profileImageElement = profilePanel.querySelector(".profile-image");
    ...
    const touched = { username: false, registerEmail: false, registerPassword: false, personalId: false };
    usernameInput.addEventListener("blur", () => { touched.username = true; validateRegistration(); });
    registerEmailInput.addEventListener("blur", () => { touched.registerEmail = true; validateRegistration(); });
    registerPasswordInput.addEventListener("blur", () => { touched.registerPassword = true; validateRegistration(); });
    personalIdInput.addEventListener("blur", () => { touched.personalId = true; validateRegistration(); });
    function validateRegistration() {
        ...
        if (touched.username) {
            ...
            usernameError.textContent = "";
            if (!usernameInput.value.trim()) { valid = false; }
        }
        if (touched.registerEmail) {
            ...
            registerEmailError.textContent = "";
            if (!registerEmailInput.value.trim()) { valid = false; }
        }
        if (touched.registerPassword) {
            ...
            registerPasswordError.textContent = "";
            if (!registerPasswordInput.value.trim()) { valid = false; }
        }
        if (touched.personalId) {
            ...
            personalIdError.textContent = "";
            if (!/^\d+[a-zA-Z]$/.test(personalIdInput.value.trim())) { valid = false; }
        }
    ...
    document.querySelectorAll('.profile-option').forEach(option => {
        option.addEventListener('click', function() {
            ...
            const profileImageElement = profilePanel.querySelector(".profile-image");
            if (profileImageElement) { profileImageElement.src = selectedImage; }
            if (profileIcon) { profileIcon.src = selectedImage; }
            const currentUser = localStorage.getItem("username");
            if (currentUser) { localStorage.setItem("profileImage_" + currentUser, selectedImage); }
        });
    });
});
